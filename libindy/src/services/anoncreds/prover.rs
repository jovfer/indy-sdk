    ...
    pub fn create_proof(&self,
        ...
        for (cred_key, (req_attrs_for_cred, req_predicates_for_cred)) in credentials_for_proving {
            ...
            let rev_state = if cred_def.value.revocation.is_some() {
                let timestamp = cred_key.timestamp.as_ref().ok_or(CommonError::InvalidStructure("Timestamp not found".to_string()))?;
                let rev_reg_id = credential.rev_reg_id.as_ref().ok_or(CommonError::InvalidStructure("Revocation Registry Id not found".to_string()))?;
